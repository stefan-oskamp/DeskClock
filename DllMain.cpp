...
// {45563D92-9D6B-4cf8-9ECE-94AA863D8555}
CLSID CLSID_DeskClock = { 0x45563d92, 0x9d6b, 0x4cf8, {0x9e, 0xce, 0x94, 0xaa, 0x86, 0x3d, 0x85, 0x55}};



HINSTANCE   g_hInst     = NULL;
unsigned long        g_cDllRef   = 0;
...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, void **ppv)
{
    ...
    if (IsEqualCLSID(CLSID_DeskClock, rclsid))
    {
        hr = E_OUTOFMEMORY;
...
HRESULT RegisterServer()
{
    ...
    StringFromGUID2(CLSID_DeskClock, szCLSID, ARRAYSIZE(szCLSID));
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (ERROR_SUCCESS == RegCreateKeyExW(HKEY_CLASSES_ROOT,
                                             ...
                                             NULL))
        {
            WCHAR const szName[] = L"DeskCloc&k";
...
HRESULT RegisterComCat()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = pcr->RegisterClassImplCategories(CLSID_DeskClock, 1, &catid);
        pcr->Release();
    }
    return hr;
}
